...
mean_g_religious = utah %>%
  group_by(Pop.Group) %>%
  summarise(mean_group_religious = mean(Religious))
...
mean_pop_utah = utah %>%
  group_by(Pop.Group) %>%
  summarise(mean_pop_utah = mean(Pop_2010))
...
Pop.Group = plyr::mapvalues(mean_g_religious$Pop.Group, from = c(1,2,3,4,5,6), 
                            to = c("group1","group2","group3","group4","group5","group6"))
mean_g_religious$Pop.Group = Pop.Group
mean_pop_utah$Pop.Group = Pop.Group

religiosity = full_join(mean_g_religious, mean_pop_utah)
...
religiosity = dplyr::arrange(religiosity, desc(religiosity$mean_pop_utah))
...
# 5.  What can you say about these relationships?
    # that as the proportion of LDS poeple goes down the proportion of Non religious,
    # people go up. as the proportion of episcopal people goes up the proportion of non religious people go up
